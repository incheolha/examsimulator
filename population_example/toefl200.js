...
router.route('/process/addTitle').post(function(req, res) {
    console.log('/process/addTitle 호출됨.');

    var paramId = req.body.id || req.query.id;
    var paramTitle = req.body.title || req.query.title;
    
    console.log('요청 파라미터 : ' + paramId + ', ' + paramTitle);
    
    if (database) {

        MemberModel.find({"id":paramId}, function(err, results) {
        
            console.log('아이디 [%s]로 사용자 검색결과', paramId);
            console.dir(results);
            
            if (results.length > 0) {  
                console.log('아이디 [%s]가 일치하는 사용자 찾음.', results.id);
            var memObjectId = results[0]._id;
            var memTitle = paramTitle;

            addTalk(database, memObjectId, memTitle, function(err, addedTitle) {
            
            if (addedTitle) {
                    console.dir(addedTitle);
                        res.writeHead('200', {'Content-Type':'text/html;charset=utf8'});
                        res.write('<h2>사용자 추가 성공</h2>');
                        res.write('<div><p>사용자 오브젝트 아이디: ' + addedTitle._id + '</p></div>');
                        res.write('<div><p>사용자 아이디 : ' + addedTitle.title + '</p></div>');
                        res.write('<div><p>이야기 이름 : ' + memObjectId + '</p></div>');
                        res.write("<br><br><a href='/public/addtalk.html'>추가하기</a>");
                    res.end();
                } else {  // 결과 객체가 없으면 실패 응답 전송
                    res.writeHead('200', {'Content-Type':'text/html;charset=utf8'});
                    res.write('<h2>이야기  추가  실패</h2>');
                    res.end();
                }
        });
        ...
        updateUser(database, paramId, paramPassword, paramName, paramStory, function(err, updatedUser) {
            ...
            if (updatedUser) {
                console.dir(updatedUser);
                    ...
                    res.write("<br><br><a href='/public/updateuser.html'>이야기 추가하기</a>");
                    res.end();
            } else {  // 결과 객체가 없으면 실패 응답 전송
                res.writeHead('200', {'Content-Type':'text/html;charset=utf8'});
                res.write('<h2>사용자 추가  실패</h2>');
...
var addTalk = function(database, memObjectId, memTitle, callback) {
    console.log('addTalk 호출됨 : ' + memObjectId + memTitle);
    
    // UserModel 인스턴스 생성
    var title = new TalkModel({"title":memTitle, "_creator":memObjectId});

    // save()로 저장 : 저장 성공 시 addedUser 객체가 파라미터로 전달됨
    title.save(function(err, addedTitle) {
        if (err) {
            callback(err, null);
            return;
        }
        console.log("사용자 데이터 추가함.");
        callback(null, addedTitle);
    });
};
